...
export const metadata: Metadata = {
  title: "Lab",
  description: `Live GitHub cartridges and recent saves from ${profile.name}.`,
};

export default async function LabPage() {
  const { gridStatus, cartridges, saves, grid } = await getGitHubSnapshot();

  return (
    <div className="space-y-6">
      <h1 className="font-pixel text-sm tracking-widest text-neon">LAB</h1>
      <p className="text-muted">
        Public cartridges, a year of commits, and the latest saves. Client work
        stays on the work page.
      </p>
      <RepoCartridges cartridges={cartridges} />
      <PowerGrid grid={grid} status={gridStatus} />
      <RecentSaves saves={saves} />
